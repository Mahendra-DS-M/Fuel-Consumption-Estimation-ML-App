...
model = joblib.load("Pickles/xgb.pkl")

# Loading Saved Ordinal Encoded files
with open('Pickles/Make_encoding.pkl', 'rb') as f:
    make_encoding = pickle.load(f)
    
with open('Pickles/Transmission_encoding.pkl', 'rb') as f:
...
st.write("Among above we got better performance for :green[Xgb Regressor].")
st.write("Xgb model saved and connected for Predictions.")
st.divider()
...
with cola:
    fbutton = st.button("Prediction for Multiple Vehicles By Uploading Csv or Excel file...", on_click=switch_fbutton_state, icon=':material/table:')
with colb:
...
if st.session_state['sbutton'] == True:
    ...
    with col2:
        vclass = st.selectbox("Select Vehicle Class:", data[data.Make==make]['Vehicle Class'].unique())

    col3, col4 = st.columns(2)
    with col3:
        esize = st.number_input("Enter Engine Size:", min_value = data['Engine Size(L)'].min(), max_value = data['Engine Size(L)'].max())
    with col4:
        cylinders = st.number_input("Enter Number of Cylinders:", min_value=data['Cylinders'].min(), max_value = data['Cylinders'].max())

    col5, col6 = st.columns(2)
    with col5:
        trans = st.selectbox("Select Transmission:", data[data.Make==make].Transmission.unique())
    with col6:
        fueltype = st.selectbox("Select Fuel Type:", data[data.Make==make]['Fuel Type'].unique())

    col7, col8, col9 = st.columns(3)
    with col7:
        co2 = st.number_input("Enter CO2 Emissions (g/km):", min_value = data['CO2 Emissions(g/km)'].min(), max_value = data['CO2 Emissions(g/km)'].max())
    with col8:
        co2r = st.number_input("Enter CO2 Rating:", min_value = data['CO2 Rating'].min(), max_value = data['CO2 Rating'].max())
    with col9:
        smogr = st.number_input("Enter Smog Rating:", min_value = data['Smog Rating'].min(), max_value = data['Smog Rating'].max())

    ################### Prediction ########################
    if st.button("Estimate"):
        data = pd.DataFrame([[make,vclass,esize,cylinders,trans,fueltype,co2,co2r,smogr]], columns=data.columns)
        row = data.copy()
